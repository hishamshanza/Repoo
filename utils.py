...
# ─────────────────────────────────────────────────────────────────────────────
# Bot API 9.4 button colour styles ("style": primary/success/danger), added to
# Telegram's Bot API on 2026-02-09. Support is detected at runtime against the
# InlineKeyboardButton constructor that's actually installed — kurigram builds
# older than the one that implemented this field simply won't have the
# parameter, and we fall back to plain buttons automatically in that case.
# See: https://core.telegram.org/bots/api#inlinekeyboardbutton
# ─────────────────────────────────────────────────────────────────────────────
def _detect_button_style_support() -> bool:
    try:
...
BUTTON_STYLES_SUPPORTED = _detect_button_style_support()

logger.info(
    "Button colour styles (Bot API 9.4): %s",
    "SUPPORTED — colour styling enabled" if BUTTON_STYLES_SUPPORTED
    else "NOT SUPPORTED by the installed pyrogram/kurigram build — buttons will render "
         "without colour. Run `pip show kurigram` and upgrade to a newer release to enable this."
)
...
# Keyword → style, matched against a button's visible label (case-insensitive).
# Order matters: DANGER is checked before SUCCESS so e.g. "Re-enable" (success)
# vs "Disable" (danger) don't collide, and both are checked before PRIMARY.
_DANGER_KEYWORDS = ("delete", "disable", "cancel", "ban", "remove", "reject", "abort", "stop")
_SUCCESS_KEYWORDS = ("enable", "confirm", "start", "browse", "unban", "yes", "approve", "continue")
_PRIMARY_KEYWORDS = ("settings", "help", "about", "back", "close", "menu")

def infer_button_style(text: str):
    """Best-effort style inference from a button's visible label.
    Returns None (→ no 'style' kwarg, i.e. today's default rendering) when
    styling isn't supported by the installed library or nothing matches."""
    if not BUTTON_STYLES_SUPPORTED or not text:
        return None
    low = text.lower()
    if any(k in low for k in _DANGER_KEYWORDS):
        return STYLE_DANGER
    if any(k in low for k in _SUCCESS_KEYWORDS):
        return STYLE_SUCCESS
    if any(k in low for k in _PRIMARY_KEYWORDS):
        return STYLE_PRIMARY
    return None

def styled_button(text: str, *args, style=None, **kwargs) -> InlineKeyboardButton:
    """Drop-in replacement for InlineKeyboardButton(text, ...) that adds a
    Bot API 9.4 colour style when the installed library supports it.
    - Never changes callback_data, url, or any other existing argument.
    - Pass style= explicitly to override auto-inference for a specific button.
    - On older libraries (BUTTON_STYLES_SUPPORTED is False) this is exactly
      equivalent to InlineKeyboardButton(text, ...) — zero behaviour change.
    """
    resolved = style if style is not None else infer_button_style(text)
    if resolved and BUTTON_STYLES_SUPPORTED:
...
async def get_bot_stats(client=None) -> dict:
    """
    Single source of truth for bot/database statistics.

    Reused by both the /stats command (plugins/p_ttishow.py) and the
    Admin Panel "Statistics" screen (plugins/admin.py) so the two never
    drift out of sync again.

    Pass `client` to also measure a real Telegram round-trip ping;
    if omitted, ping is reported as unavailable instead of a fake 0ms value.
    """
    import time
    import psutil
    ...
    db_type = "MongoDB" if getattr(db, "use_mongo", True) else "PostgreSQL"
    ...
    return {
        ...
        "db_size_readable": humanbytes(db_size_bytes),
        "cpu_percent": cpu_percent,
        "ram_percent": ram.percent,
        "ram_used_readable": humanbytes(ram.used),
        "ram_total_readable": humanbytes(ram.total),
